...
function mergeCategory(categories, impCat, replace) {
  const idx = categories.findIndex((c) => c.id === impCat.id);
  if (idx < 0) {
  ...
  if (replace) {
    categories[idx] = impCat;
    return;
  }

  const existingCodes = new Set(impCat.products.map((p) => p.id));
  const merged = [
    ...categories[idx].products.filter((p) => !existingCodes.has(p.id)),
    ...impCat.products,
  ];
  categories[idx] = { ...impCat, products: merged };
}
...
export function mergeImportedCatalog(existing, imported, { replace = true } = {}) {
  ...
  for (const impCat of imported.categories || []) {
    mergeCategory(catalog.categories, impCat, replace);
  }
